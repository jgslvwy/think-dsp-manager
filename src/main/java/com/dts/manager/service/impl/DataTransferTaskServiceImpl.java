...
public class DataTransferTaskServiceImpl implements DataTransferTaskService {
    ...
    public Mono<Boolean> delete(String taskId) {
        ...
        boolean determineIfDelete = Objects.equals(dataTask.getRunState(), RunState.RUNING) || Objects.equals(dataTask.getRunState(), RunState.END);
    ...
    public Mono<Boolean> start(DataTask dataTask) {
        boolean determine = Objects.equals(dataTask.getRunState(), RunState.RUNING) || Objects.equals(dataTask.getRunState(), RunState.END);
        if (determine) {
            ...
            ThreadPoolTaskExecutor asyncServiceExecutor = (ThreadPoolTaskExecutor) applicationContext.getBean("asyncServiceExecutor");
            if (null == asyncServiceExecutor) {
                throw new RuntimeException("this executor is null");
            }
            asyncServiceExecutor.submitListenable(ProcessorFactory.getInstance().create(toEvent(dataTask)));
            return Mono.just(Boolean.TRUE);
        }
    }
